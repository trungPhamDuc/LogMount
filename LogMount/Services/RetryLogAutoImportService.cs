...
public class RetryLogAutoImportService : BackgroundService
{
    ...
    private async Task ReimportPreviousDayIfDueAsync(DateTime now, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var validEntries = entries.Where(RetryLogAnalysisService.IsRealError).ToList();
            if (validEntries.Count == 0)
            {
                throw new InvalidOperationException("Không thay thế dữ liệu ngày trước bằng tệp không có dòng RetryLog hợp lệ.");
            }
